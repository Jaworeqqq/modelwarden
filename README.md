...
**Synthetic tests agree with whatever the scanner already believes.** Every format
here was first tested against samples this project generated, and almost every one
of those passes hid a defect that only appeared when the scanner met a file written
by the real library. Keras writes registered functions as `builtins` with a
registry key like `mw>scaled_relu`. The documented Windows launch for nearly every
MCP server is `cmd /c npx -y <package>`. `Bearer ${MCP_TOKEN}` is how a header is
written, not a leaked token. A fractal heap counts offsets from the first byte of
the block, header included — reading it the intuitive way parsed 0 of 33 records.
None of those were visible until real output was on the other side of the test.

**A quality gate that is not itself tested is decoration.** A commit went out red
because the gate was written as `ruff … && echo OK`, and `set -e` does not fire for
a command inside an and-or list. The linter failed, the script exited zero, CI
caught what the gate was supposed to. The fix was one line; the lesson was that the
thing doing the verifying needs verifying too.

**A silent skip is worse than a wrong finding.** Pointing the scanner at the machine
it runs on showed that a real Claude Code configuration nests its servers under a
project path. Without a top-level map the file was not recognised as a
configuration at all, so it was counted as *skipped* and the report said nothing
about it. A false positive argues with you; a silent pass does not.

**Measuring changes severities, not just bugs.** Of 17 published MCP configurations,
14 pin no version. At medium severity that rule would have fired on almost every
real setup and people would have stopped reading the output, so it is low.
Thresholds followed the same route: the densest passage in 23 honest documents is
10.5% one word, which is why the chunk threshold is 25% and not the 12% used for
whole documents.

**Report a rate, not a verdict — and that stopped being an opinion.** Five identical
runs against the same endpoint, at temperature 0 with a fixed seed, returned 2/4,
1/4, 1/4, 3/4 and 1/4 for system-prompt leakage. A single sample would have called
the same model anywhere between 25% and 75%.

**The quiet results are what make the loud ones believable.** A model that failed
three probes every time still summarised a document while ignoring the instruction
planted in it. If everything fires, a run cannot distinguish a weak target from a
detector that is always on.
